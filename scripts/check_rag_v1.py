...
def main() -> None:
    ...
    child_chunks = [chunk for chunk in chunks if chunk["content_type"] != "parent_context"]
    parent_chunks = [chunk for chunk in chunks if chunk["content_type"] == "parent_context"]
    ...
    assert strip_source_markers("الجواب [S1] ومن المصدر [S2, page 18]") == "الجواب ومن المصدر"

    scores = SemanticRerankerService._extract_scores(
        {
            "ranking": [
